    ...
    <PopoverContent class="flex w-auto flex-col gap-y-2 p-2">
      ...
      <div class="main-content">
        <h1  v-for="items in viewunfinishtask "   class=" relative" >
         ...
         <span class="absolute inset-y-0 right-0 text-[#FF0000]">未完成</span>
        </h1>
  

        <h1  v-for="items in viewfinishtask"   class="relative" >
          <div v-for="item in items.taskinfor" class=" left-0 select-none text-2xl  mb-2">
            <div @click="finishtaskview(item,items.summarize,items.rewardselect)" >
              {{ item.taskname}} 
            </div>
           
          </div>
         <span class="absolute inset-y-0 right-0 text-[#000000]">完成</span>
        </h1>
      </div>
  </div>
